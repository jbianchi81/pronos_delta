...
from Funciones import getLastProno, getObs, eliminaSaltos, prono2serie
from Funciones import uploadPronoSeries, prono2json, outputjson, outputcsv
...
def runPlan(plan,plots=True,upload=False,output_csv=False):
    ...
    if plan in ("all","All","ALL"):
        for key, config in config_estaciones.items():
            if "desactivar" in config and config["desactivar"] == True:
